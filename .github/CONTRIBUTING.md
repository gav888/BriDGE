...
Please open a [GitHub issue](https://github.com/gav888/BriDGE/issues) using
the bug report template. The most useful reports include:

- a minimal reproducible example (ideally using `bridge_generate_data()`)
...
[roadmap issues](https://github.com/gav888/BriDGE/issues?q=is%3Aissue+label%3Aroadmap)
...
2. Make your changes. Please:
   ...
   - update documentation (roxygen comments; run `devtools::document()`),
...
BriDGE implements the protocol from Veltri & Banerjee (in press, *Behavior
Research Methods*). Methodological extensions (new estimators, new discovery
constraints) are welcome but should stay consistent with the paper's
framework — open an issue to discuss before investing significant effort.
